...
def register():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            flash("You've already signup with this username, Please login")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user:
            hash_pass = check_password_hash(pwhash=user.password, password=password)
            if hash_pass:
                login_user(user)
                return redirect(url_for("get_all_posts"))
            else:
                flash("Invalid password, Please enter valid password")
                return redirect(url_for("login"))
        else:
            flash("User doesn't exists, Please enter valid username")
            return redirect(url_for("login"))
